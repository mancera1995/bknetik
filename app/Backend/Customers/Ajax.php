...
class Ajax extends \BookneticApp\Providers\Core\Controller
{
	...
	public function save_customer()
	{
        ...
        if ( $isEdit && $customerOnMultiTenants && ($getOldInf->email != $email || boolval($allow_customer_to_login) != email_exists( $getOldInf->email ) || ! empty( $wp_user_password )) )
        {
		...
		if( ( ! $isEdit || $email != $getOldInf->email ) && ( email_exists( $email ) !== false || username_exists( $email ) !== false || ( $email != '' && Customer::where( 'email', $email )->count() > 0 ) ) )
		{
