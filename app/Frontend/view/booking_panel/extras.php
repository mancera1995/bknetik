...
else
{
	...
	foreach ( $parameters['extras'] AS $eq => $extraInf )
	{
        ...
        if ( Helper::getOption('collapse_service_extras', 'off')=='on' && array_key_exists( $extraInf[ 'category_id' ], $parameters['extra_categories'] ) && ! in_array( $extraInf[ 'category_id' ], $printedCategories ) )
        {
        ...
        else if ( Helper::getOption('collapse_service_extras', 'off')=='off' && $i == 0 )
        {
            $i++;
		...
		?>
        <div class="booknetic_service_extra_card <?php echo $extraInf['max_quantity'] == 1 ? ' booknetic_extra_on_off_mode' : ''?> <?php echo (int)$extraInf["min_quantity"] > 0 ? 'booknetic_service_extra_card_selected' : '' ?>" data-id="<?php echo (int)$extraInf['id']?>" style="<?= array_key_exists( $extraInf[ 'category_id' ], $parameters['extra_categories'] ) ? 'display:none;' : '' ?>" >
