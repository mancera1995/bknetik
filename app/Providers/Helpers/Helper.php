...
class Helper
{
    ...
    private static function getTranslationsCache( ) {
        if ( empty( self::$translationsCache ) && function_exists('wp_get_current_user') && ! Permission::isBackEnd() )

        {
            self::$translationsCache = ["pending"];
    ...
    public static function getURLOfUsersDashboard( $user = null )
    {
        ...
        {
            $redirect_url = Helper::_post('redirect','','string');
            if( empty( $redirect_url ) )
            {
                // doit
                $redirect_url = self::customerPanelURL();
            }
        }
        else
        {
