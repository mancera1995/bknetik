...
class Config
{
	...
	public static function registerTextDomain()
	{
		add_action( 'plugins_loaded', function()
		{
            ...
            if ( Helper::isSaaSVersion() && ! Permission::isSuperAdministrator() && Permission::tenantId() > 0 )
            {
                ...
                $checkTenantExist = null;

                if( !empty( $currentDomain ) )
                    $checkTenantExist = \BookneticSaaS\Models\Tenant::where('domain', $currentDomain)->fetch();

                if ( !is_null( $checkTenantExist ) )
                    $tenantId = $checkTenantExist->id;
            }

            if( Helper::isSaaSVersion() && ! Permission::isSuperAdministrator() && Permission::tenantId() > 0 && file_exists( WP_PLUGIN_DIR . '/' . $path . '/' . $tenantId . '/booknetic-' . get_locale() . '.mo' ) )
			{
				$path .= '/' . $tenantId;
			}
    ...
    public static function registerCoreWorkflowEvents()
    {
        ...
        self::$workflowEventsManager->get('customer_forgot_password')
            ->setTitle(bkntc__('Customer resets password'))
            ->setAvailableParams(['customer_id']);
        ...
        add_action('bkntc_payment_confirmed', function ($appointmentId)
        {
            ...
            self::$workflowEventsManager->trigger('booking_new', [
	            ...
	            if ( isset($data['called_from']) && !empty( $data['called_from'] ) && $data['called_from'] == 'backend' && !Permission::isBackEnd() )
					return false;

	            if ( isset($data['called_from']) && !empty( $data['called_from'] ) && $data['called_from'] == 'frontend' && Permission::isBackEnd() )
		            return false;
